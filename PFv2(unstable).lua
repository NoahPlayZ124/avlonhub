if Drawing and getgc and writefile and readfile then
    ...
    local visuals_chamstransparency
    ...
    local ESP_ChamsTransparency = ESP_Window:AddSlider("Chams Transparency", function(trans)
        ...
        if trans ~= 0 then
            Main_Settings.visuals.chamstransparency = trans
        end
	end, { 
		["min"] = 0, 
		["max"] = 100, 
    ...
    ESP_ChamsTransparency:Set(Main_Settings.visuals.chamstransparency)
					...
					for i2,v2 in pairs(v:GetChildren()) do
						pcall(function()
							if (v2:IsA("BasePart") or (v2:IsA("Model") and v2:FindFirstChild("Slot1"))) and not v2:FindFirstChild("BoxHandleAdornment") and v2.Name ~= "HumanoidRootPart" then
								...
								adornment.Transparency = visuals_chamstransparency
							...
							elseif v2:FindFirstChild("BoxHandleAdornment") then
                                ...
                                c.Size = v2.Size
                                c.Transparency = visuals_chamstransparency
								if esp_rainbow then
    ...
    game:GetService('RunService').RenderStepped:connect(function()
        if Aimbot_ENABLED then 
            ...
            if game.Players.LocalPlayer.Team.Name ~= Target.Parent.Name and Main_Settings.aimbot.enabled == true then
                if  Main_Settings.aimbot.wallcheck then
                    if WallChecker(Target[Main_Settings.aimbot.aimat].Position, Target) == true then
	...
	Mods_NoSpreadToggle:Set(Main_Settings.mods1.nospread)
		...
		--Rapid Fire
	local Mods_RapidFireToggle = Mods_Window:AddSwitch("Rapid Fire", function(bool)
	...
	Mods2_NoSwayToggle:Set(Main_Settings.mods2.nosway)
		...
		--Inf Penetration
	local Mods2_InfPenetrationToggle = Mods2_Window:AddSwitch("Infinite Penetration", function(bool)
	    Main_Settings.mods2.infpenetration = bool
		if bool then
			for i,v in pairs(game.ReplicatedStorage.GunModules:GetChildren()) do
				local mod = require(v) 
				--Inf Penetration
				mod["penetrationdepth"] = 1000
			end
		elseif bool == false then
			for i,v in pairs(game.ReplicatedStorage.GunModules:GetChildren()) do
				local mod = require(v);
				local backupmod = require(backupModules[v.Name]);
				--Inf Penetration
				mod["penetrationdepth"] = backupmod["penetrationdepth"]
			end
		end
	end)
	Mods2_InfPenetrationToggle:Set(Main_Settings.mods2.infpenetration)
	
		--Inf Ammo
	local Mods2_InfAmmoToggle = Mods2_Window:AddSwitch("Infinite Ammo", function(bool)
	    Main_Settings.mods2.infammo = bool
		if bool then
			for i,v in pairs(game.ReplicatedStorage.GunModules:GetChildren()) do
				local mod = require(v) 
				--Inf Ammo
				mod["sparerounds"] = math.huge
				mod["magsize"] = math.huge
			end
		elseif bool == false then
			for i,v in pairs(game.ReplicatedStorage.GunModules:GetChildren()) do
				local mod = require(v);
				local backupmod = require(backupModules[v.Name]);
				--Inf Ammo
				mod["sparerounds"] = backupmod["sparerounds"]
				mod["magsize"] = backupmod["magsize"]
			end
		end
	end)
	Mods2_InfAmmoToggle:Set(Main_Settings.mods2.infammo)
	
		--Inf grenades
	local Mods2_AllAutoToggle = Mods2_Window:AddButton("Infinite Grenades", function(bool)
		_G.Mods2_infgrenades = true
	end)
	
	spawn(function()
	while wait() do
	if _G.Mods2_infgrenades then
	...
	local Other_SpeedSlider = Other_Window:AddSlider("Walk Speed", function(x)
		Other_Speed = x
		if x ~= Main_Settings.mods1.firerate then
		    Main_Settings.mods1.firerate = x
		end
	end, {
		["min"] = 0,
	...
	local Other_JumpSlider = Other_Window:AddSlider("Jump Power", function(x)
		Other_Jump = x * 2
		if x ~= Main_Settings.mods1.jumppower then
		    Main_Settings.mods1.jumppower = x
		end
	end, { 
		["min"] = 0, 
		["max"] = 100, 
